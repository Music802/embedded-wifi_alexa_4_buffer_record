    ...
    switch ( event_header->event_type )
    {
        case WLC_E_PSK_SUP:
            ...
            if ( event_header->status == WLC_SUP_KEYED )
            {
                ...
                wiced_join_status[event_header->interface] &= ~(JOIN_EAPOL_KEY_M1_TIMEOUT | JOIN_EAPOL_KEY_M3_TIMEOUT | JOIN_EAPOL_KEY_G1_TIMEOUT | JOIN_EAPOL_KEY_FAILURE);
            }
            else
            {
...
wwd_result_t wwd_wifi_get_cap( char *buf, uint16_t buflen, char *cap )
{
    ...
    if ( cap && strnstr( buf, cap, buflen) == NULL )
    {
