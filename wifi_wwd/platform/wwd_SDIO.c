 ...
 ******************************************************/
void  host_platform_sdio_irq_callback(USDHC_Type *base, void *userData);
...
static void host_platform_wifi_oob_irq_init(void)
{
    gpio_pin_config_t pinconfig = {.direction = kGPIO_DigitalInput,
				   .outputLogic = 0U,
				   .interruptMode = kGPIO_IntRisingEdge };

    NVIC_SetPriority(BOARD_WL_OOB_IRQ, 6U);
    EnableIRQ(BOARD_WL_OOB_IRQ);
    // Initialize GPIO Pin
    GPIO_PinInit(BOARD_WL_OOB_IRQ_port, BOARD_WL_OOB_IRQ_pin, &pinconfig);
    /* Enable GPIO pin interrupt */
    GPIO_PortEnableInterrupts(BOARD_WL_OOB_IRQ_port,1U << BOARD_WL_OOB_IRQ_pin);
}
	

wwd_result_t host_enable_oob_interrupt( void )
{
    host_platform_wifi_oob_irq_init();
    return WWD_SUCCESS;
}
...
wwd_result_t host_platform_enable_high_speed_sdio( void )
{
	...
	/* default mode 25MHZ */
	g_sdio.busClock_Hz = SDMMCHOST_SET_CARD_CLOCK(g_sdio.host.base, g_sdio.host.sourceClock_Hz, SD_CLOCK_25MHZ);

	if( !g_sdio.busClock_Hz )
	{
		WPRINT_WWD_INFO(("\n Switch to high speed fail \n"));
		return  (wwd_result_t)kStatus_SDMMC_SDIO_SwitchHighSpeedFail;
	}
...
#ifndef WICED_DISABLE_MCU_POWERSAVE
void BOARD_WL_OOB_IRQ_Handler (void)
{
    oob_irq_cnt++;
    /* clear the interrupt status */
    GPIO_PortClearInterruptFlags( BOARD_WL_OOB_IRQ_port, 
                                  1U << BOARD_WL_OOB_IRQ_pin );
    wwd_thread_notify_irq();
}
#endif
