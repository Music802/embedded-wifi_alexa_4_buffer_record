...
#define PDM_SAI_CLK_FREQ (CLOCK_GetFreq(kCLOCK_AudioPllClk) / (PDM_SAI_CLOCK_SOURCE_DIVIDER + 1U) / (PDM_SAI_CLOCK_SOURCE_PRE_DIVIDER + 1U))
...
pdm_mic_status_t PDM_MIC_GetSAIConfiguration(uint8_t channelMask, uint32_t *burstSize, uint32_t *burstBytes, uint32_t *startIdx)
{
...
pdm_mic_status_t PDM_MIC_ConfigMic(pdm_mic_handle_t *handle)
{
    ...
    uint8_t channelMask = handle->config->saiChannelMask;
