...
#if (defined(SHELL_USE_COMMON_TASK) && (SHELL_USE_COMMON_TASK > 0U))
    /*
     * Post an additional SHELL_Task command in order for the exit flag to
     * be checked and the SerialManager handles to be closed.
     */
       shellContextHandle->commontaskMsg.callback = SHELL_Task;
       shellContextHandle->commontaskMsg.callbackParam = shellHandle;
       COMMON_TASK_post_message(&shellContextHandle->commontaskMsg);
#endif
#else
       SHELL_Task(shellHandle);
#endif
